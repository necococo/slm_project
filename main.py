...
def main():
    ...
    try:
        ...
        if device.type == "cpu":
            ...
            train_dataset = train_dataset.map(tokenize_fn, batched=True, batch_size=4096, num_proc=8, remove_columns=["text"])
            valid_dataset = valid_dataset.map(tokenize_fn, batched=True, batch_size=4096, num_proc=8, remove_columns=["text"])
            test_dataset = test_dataset.map(tokenize_fn, batched=True, batch_size=4096, num_proc=8, remove_columns=["text"])
