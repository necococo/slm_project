...
def main():
    ...
    try:
        ...
        if device.type == "cpu":
            ...
            train_dataset = train_dataset.map(tokenize_fn, batched=True, batch_size=4096, num_proc=32, remove_columns=["text"]) # 32と書いても動くが多分８で動いている
            valid_dataset = valid_dataset.map(tokenize_fn, batched=True, batch_size=4096, num_proc=32, remove_columns=["text"])
            test_dataset = test_dataset.map(tokenize_fn, batched=True, batch_size=4096, num_proc=32, remove_columns=["text"])
