...
# -*- coding: utf-8 -*-
"""
高効率なWavelet Transformerモデルの学習スクリプト
マスク言語モデリング (MLM) による事前学習を実行
"""
...
from wavelet_transformer.config import get_wavelet_config
...
def parse_args():
    parser = argparse.ArgumentParser(description="Wavelet Transformerモデルの学習")
    ...
    parser.add_argument("--tokenizer", type=str, default="bert-base-uncased",
                        help="使用するトークナイザー")
    parser.add_argument("--attention_type", type=str, default="svd", 
                        choices=["svd", "linear", "fft"],
                        help="使用する注意機構のタイプ")
...
def main():
    ...
    # 設定取得
    config = get_wavelet_config(args.model_size)
    ...
    # ロガー設定
    logger = setup_logging(os.path.join(config.log_dir, "wavelet_training.log"))
    logger.info(f"Wavelet Transformer学習を開始します - サイズ: {args.model_size}, 注意機構: {args.attention_type}")
    ...
    # モデルの初期化
    logger.info("Wavelet Transformerモデルを初期化中...")
